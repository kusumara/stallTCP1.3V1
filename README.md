...
**本系统支持“上游订阅”与“自建节点”的智能合并：**
*   **共存逻辑**：如果您同时配置了 **`SUB_DOMAIN`** (上游订阅) 和 **`ADD` / `ADDAPI` / `ADDCSV`** (自建节点)。
*   **自动聚合**：系统会自动抓取上游订阅的所有节点，并与您的自建节点、内置节点进行**合并**。
*   **最终效果**：客户端获取的订阅链接将包含：`上游节点` + `自建节点` + `内置节点` 的总和。
